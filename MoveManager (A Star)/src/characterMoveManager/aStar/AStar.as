package characterMoveManager.aStar
{
	import common.ICell;
	import common.IGrid;
	...
	public class AStar
	{
		...
		private function getNeighbours(square:Square):Vector.<Square>
		{
			...
			if (_grid.getCell(square.x + 1, square.y) != 0 && _grid.getCell(square.x + 1, square.y) == AbstractField.STATE_EMPTY && !isInCloseList(new Square(square.x + 1, square.y)))				// x + 1, y
				neighbours.push(new Square(square.x + 1, square.y));
			if (_grid.getCell(square.x + 1, square.y + 1) != 0 && _grid.getCell(square.x + 1, square.y + 1) == AbstractField.STATE_EMPTY && !isInCloseList(new Square(square.x + 1, square.y + 1)))	// x + 1, y + 1
				neighbours.push(new Square(square.x + 1, square.y + 1));
			if (_grid.getCell(square.x - 1, square.y) != 0 && _grid.getCell(square.x - 1, square.y) == AbstractField.STATE_EMPTY && !isInCloseList(new Square(square.x - 1, square.y)))				// x - 1, y
				neighbours.push(new Square(square.x - 1, square.y));
			if (_grid.getCell(square.x, square.y - 1) != 0 && _grid.getCell(square.x, square.y - 1) == AbstractField.STATE_EMPTY && !isInCloseList(new Square(square.x, square.y - 1)))				// x, y - 1
